...
export function TransactionForm() {
  ...
  // Clipboard Paste (Ctrl + V / Cmd + V) Support for Screenshots & Images
  useEffect(() => {
    const handleWindowPaste = (e: ClipboardEvent) => {
      ...
      if (extractedFiles.length > 0) {
        e.preventDefault();
        const newStaged: StagedFormAttachment[] = extractedFiles.map(file => ({
          nama: file.name,
          tipe: file.type || 'image/jpeg',
          dataUrl: '',
          previewUrl: URL.createObjectURL(file),
            ...
            {stagedFiles.length === 0 ? (
              ...
              >
                ...
                <div className="flex flex-wrap items-center justify-center gap-1.5 pt-1">
                  <span className="inline-flex items-center gap-1 text-[10.5px] font-bold text-emerald-700 bg-emerald-100 px-2 py-0.5 rounded-lg border border-emerald-200">
                    📋 Bisa langsung Ctrl + V (Paste Screenshot)
                  </span>
                  <span className="text-[10px] text-gray-400">Foto & Dokumen PDF didukung</span>
                </div>
              </label>
            ) : (
